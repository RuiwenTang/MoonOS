...
static void balloc_add_range(struct balloc_ranges* rs, uintptr_t begin,
                             uintptr_t end) {
    size_t from = 0, to;
    ...
    if (rs->size + 1 - (to - from) > BALLOC_MAX_RANGE_SIZE) {
        kprintf("There is no enought space,"
...
static void balloc_remove_range(struct balloc_ranges* rs, uintptr_t begin,
                                uintptr_t end) {
    size_t from = 0, to;
...
static uintptr_t balloc_find_range(const struct balloc_ranges* rs,
                                   uintptr_t from, uintptr_t to, size_t size,
                                   size_t align) {
    for (size_t i = 0; i != rs->size; i++) {
        ...
        const uintptr_t addr = balloc_align_up(rs->range[i].begin, align);
...
uintptr_t __balloc_alloc(uintptr_t from, uintptr_t to, size_t size,
                         size_t align) {
...
void balloc_setup(const struct multiboot_info* info) {
    ...
    while (ptr + sizeof(struct multiboot_mmap_entry) <= end) {
        const multiboot_memory_map_t* entry =
                (const multiboot_memory_map_t*)va(ptr);
        const uint64_t from = entry->addr;
        const uint64_t to = from + entry->len;
...
size_t balloc_free_ranges(void) { return free.size; }

void balloc_get_free_range(size_t i, struct balloc_range* range) {
    *range = free.range[i];
}

uintptr_t balloc_phys_mem(void) { return all.range[all.size - 1].end; }