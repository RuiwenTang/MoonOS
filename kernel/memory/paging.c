 ...
 **/
static void __pt_map(pte_t* pt, uint64_t virt, uint64_t size, uint64_t phys,
...
#define MIN(a, b) ((a) < (b) ? (a) : (b))
    for (int i = from; i != to; i++) {
        ...
        __pt_map(va(pte & PTE_PHYS_MASK), virt, tomap, phys, pte_flags,
                 lvl - 1);
...
static void pt_map(pte_t* pt, uintptr_t virt, uintptr_t size, uintptr_t phys,
                   pte_t flags) {
