...
static void handle_exception(struct frame* frame, int exception) {
    ...
    kprintf("RIP 0x%llx, RSP 0x%llx, RBP 0x%llx,\n"
            ...
            frame->rip, frame->rsp, frame->rbp, frame->r8, frame->r9,
            frame->r10, frame->r11, frame->r12, frame->r13, frame->r14,
            frame->r15, frame->rax, frame->rbx, frame->rcx, frame->rdx,
            frame->rsi, frame->rdi);
    while (1)
        ;
}

static void end_of_interrupt(void) { local_apic_write(LOCAL_APIC_EOI, 0); }
...
void isr_handler(struct frame* frame) {
    ...
    else if (irq == INTNO_SPURIOUS)
        handle_spurious();
    else
        handle_interrupt(irq - IDT_EXCEPTIONS);
}
...
static void idt_setup(void) {
    ...
    const struct idt_ptr ptr = {.limit = sizeof(IDT) - 1,
                                .base = (uint64_t)IDT};
